...
const UserCrops = {
  ...
  open(id){
    ...
    let h = '<p class="muted sm" style="margin-top:0">Everything the app does for a built-in crop it will do for this one — spacing, watering, the calendar, rotation, companions. The only difference is that it will always be shown as <b>your figures</b> rather than an extension service\'s.</p>';

    h += '<div class="grid2"><div><label class="f">Name</label>' +
      '<input type="text" id="uc-name" value="' + esc(v("name","")) + '" placeholder="Lemongrass"></div>' +
